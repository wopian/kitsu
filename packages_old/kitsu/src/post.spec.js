...
describe('kitsu', () => {
  describe('post', () => {
    ...
    it('sends data in request', async () => {
      ...
      await expect(
        await api.post('anime', { type: 'anime', name: 'Name' })
      ).toBeUndefined()
    })
    ...
    it('handles nested routes', async () => {
      ...
      await expect(
        await api.post('something/1/relationships/anime', {
          type: 'anime',
          name: 'Name'
        })
      ).toBeUndefined()
    ...
    it('throws an error if missing a JSON object body', async () => {
      ...
      try {
        ...
        expect(error.message).toEqual('POST requires an object or array body')
      }
    })
  })
})
