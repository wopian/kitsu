...
describe('kitsu', () => {
  describe('get', () => {
    it('uses provided axios options', async () => {
      ...
      expect(api.axios.get).toHaveBeenCalledWith(
        'anime',
    ...
    it('fetches a single resource', async () => {
      ...
      const api = new Kitsu()
      mock
        .onGet(`anime/${getSingle.jsonapi.data.id}`)
        .reply(200, getSingle.jsonapi)
    ...
    it('fetches a single resource with includes', async () => {
      ...
      const api = new Kitsu()
      mock
        .onGet(`anime/${getSingleWithIncludes.jsonapi.data.id}`, {
          include: 'author,comments'
        })
        .reply(200, getSingleWithIncludes.jsonapi)
      const request = await api.get('anime/1', {
    ...
    it('fetches a single resource with a camelCase relationship include', async () => {
      ...
      const api = new Kitsu()
      mock
        .onGet('anime/1', { params: { include: 'animeStaff' } })
        .reply(200, getSingleWithIncludes.jsonapi)
      const request = await api.get('anime/1', {
    ...
    it('fetches :resource/:id/relationships/:relationship', async () => {
      ...
      const api = new Kitsu()
      mock
        .onGet('media-relationships/1/relationships/destination')
        .reply(200, response)
      const request = await api.get(
        'mediaRelationships/1/relationships/destination'
      )
    ...
    it('fetches infinitely nested relationship resource', async () => {
      ...
      const api = new Kitsu()
      mock
        .onGet('printers', { params: { include: 'paper,labels' } })
        .reply(200, {
          data: [
            {
              type: 'printer',
      ...
      const request = await api.get('printers', {
        params: { include: 'paper,labels' }
      })
