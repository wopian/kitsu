...
import { getSingleWithIncludes } from 'specification'
...
describe('kitsu', () => {
  describe('request', () => {
    it('uses provided axios options', async () => {
      ...
      expect(api.axios.request).toHaveBeenCalledWith(
    ...
    it('makes PATCH requests (array)', async () => {
      ...
      mock.onPatch('anime').reply(200, {
        data: [genericResponse.data, genericResponse.data]
      })
      const request = await api.request({
        method: 'patch',
        url: 'anime',
        type: 'anime',
        body: [genericResponse.data, genericResponse.data]
      })
      expect(request).toEqual({
        data: [genericRequest.data, genericRequest.data]
      })
    })
    ...
    it('throws an error if body is missing', async () => {
      ...
      try {
        ...
        expect(error.message).toEqual('PATCH requires a resource type')
      }
    })
