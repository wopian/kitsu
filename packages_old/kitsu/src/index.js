...
export default class Kitsu {
  constructor(options = {}) {
    this.camel = options.camelCaseTypes === false ? s => s : camel
     ...
     *
     */
    this.plural = options.pluralize === false ? s => s : pluralise
  ...
  async get(model, config = {}) {
    try {
      ...
      return responseHeaders
        ? { ...deserialise(data),  headers: responseHeaders  }
        : deserialise(data)
    } catch (error_) {
      throw error(error_)
    }
  ...
  async self(config = {}) {
    try {
      const headers = { ...this.headers, ...config.headers }
      const parameters = { ...config.params,  filter: { self: true }  }
      ...
      return res.headers
        ? {  data: res.data[0] ,  headers: res.headers  }
        : { data: res.data[0] }
    } catch (error_) {
      throw error(error_)
    }
