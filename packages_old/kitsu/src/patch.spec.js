...
import { patchSingle, patchSingleMissingID } from 'specification'
...
describe('kitsu', () => {
  describe('patch', () => {
    it('uses provided axios options', async () => {
      ...
      expect(api.axios.patch).toHaveBeenCalledWith(
        'anime/1',
    ...
    it('sends data in request', async () => {
      ...
      await expect(
        await api.patch('post', { id: '1', content: 'Hello World' })
      ).toBeUndefined()
    })
    ...
    it('throws an error if missing a JSON object body', async () => {
      ...
      try {
        ...
        expect(error.message).toEqual('PATCH requires an object or array body')
      }
    })
    ...
    it('throws an error if ID is missing (patch)', async () => {
      ...
      try {
        ...
        expect(error.message).toEqual('PATCH requires an ID for the posts type')
      }
    })
    ...
    it('updates a single resource', async () => {
      ...
      const api = new Kitsu({ headers: { Authorization: true } })
      mock
        .onPatch(`posts/${patchSingle.jsonapi.data.id}`)
        .reply(200, patchSingle.jsonapi)
    ...
    it('handes nested routes', async () => {
      ...
      mock
        .onPatch(
          `something/1/relationships/posts/${patchSingle.jsonapi.data.id}`
        )
        .reply(200, patchSingle.jsonapi)
      const request = await api.patch(
        'something/1/relationships/posts',
        patchSingle.kitsu
      )
      expect(request).toEqual({ data: patchSingle.kitsu })
    })
