...
describe('kitsu-core', () => {
  describe('deattribute', () => {
    it('deattributes an object', () => {
      expect.assertions(1)
      expect(deattribute(attributesKey.jsonapi)).toEqual(attributesKey.kitsu)
    })

    it('deattributes an array of objects', () => {
      expect.assertions(1)
      expect(
        deattribute([attributesKey.jsonapi, attributesKey.jsonapi])
      ).toEqual([attributesKey.kitsu, attributesKey.kitsu])
    })

    it('does nothing if no attributes supplied', () => {
      expect.assertions(1)
      expect(deattribute({ id: '1' })).toEqual({ id: '1' })
    })

    it('strips attributes object if empty', () => {
      expect.assertions(1)
      expect(deattribute({ id: '1', attributes: {} })).toEqual({ id: '1' })
    })

    it('does nothing if data is missing', () => {
      expect.assertions(1)
      expect(deattribute()).toBeUndefined()
    })
